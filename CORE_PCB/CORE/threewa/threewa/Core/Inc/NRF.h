...
void NRF_LowPower_Mode(void);	
 void NRF_dispose(void);//处理接收过来的数据;
 struct NRF_BUFFER
{
	uint16_t one; //右侧摇杆左右，会自动回中，中间值约为2039
	uint16_t two; //右侧摇杆上下，不会自动回中，需要给死区
	uint16_t three;//左侧摇杆上下，中间死去1990-1940（1960）
	uint16_t four;//左侧摇杆左右，中间死区为1800-1860（1840）
	uint16_t satus;//状态，1是原地根据偏航角旋转，2是小车方向不变进行四周移动，3是小车一边旋转一边移动，4是小车没有定下方向，只移动最普通的
	uint16_t left_right;//当前读取左边还是右边的摇杆，1是左边，2是右边，3是左右一起读取
};
